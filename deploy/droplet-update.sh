#!/usr/bin/env bash
#
# droplet-update.sh — Pull new code and restart the app on a DigitalOcean Droplet.
#
# Run as root after SSH'ing into the Droplet:
#     bash deploy/droplet-update.sh
#
# Assumes droplet-deploy.sh has been run at least once on this server.
#
...
if [[ "$(id -u)" -ne 0 ]]; then
    ...
    yellow "Try: sudo bash deploy/droplet-update.sh"
    exit 1
fi
...
if [[ ! -d "$VENV_DIR" ]]; then
    ...
    yellow "Looks like a first-time setup — run deploy/droplet-deploy.sh instead."
    exit 1
fi
